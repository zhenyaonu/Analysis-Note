...
\begin{theorem}[{\bf Brunn–Minkowski}]\label{theorem_130}
...
\begin{definition}
If $E \subset \mathbb{R}^n$ is compact, then its closed convex hull $\co(E)$ of $E$ is defined as the intersection of all convex closed sets that contain $E$.
\end{definition}

\medskip

Clearly, $\co(E)$ is convex, compact and is the smallest convex set that contains $E$. Also, $\diam E = \diam (\co(E))$.
...
\begin{align*}
    \mu(E) \leq \mu(\co(E)) \leq \frac{\omega_n}{2^n} (\diam E)^n,
\end{align*}
where $\omega_n$ is the volume of the unit ball in $\mathbb{R}^n$, see Theorem \ref{theorem_B4}.
\end{theorem}
\begin{proof}
It suffices to assume tat $E$ is convex. Indeed, if we know the result for convex sets, then for any $E \subset \mathbb{R}^n$, since $\diam E = \diam (\co(E))$, we have
\begin{align*}
    \mu(E) \leq \mu(\co(E)) \leq \frac{\omega_n}{2^n} (\diam (\co(E)))^n = \frac{\omega_n}{2^n} (\diam E)^n.
\end{align*}
...
\begin{enumerate}[label=(\alph*)]
    \item First, we prove that $\H^n_{\delta} (A) \leq \L_n^*(A)$. By Theorem \ref{theorem_122}, there is an open set $U \subset \mathbb{R}^n$ such that $A \subset U$ and $\L_n(U) \leq \L_n^*(A) + \varepsilon$. By Vitali covering theorem, $U$ can be covered a countable collection of closed balls $\{\overline{B}_i\}$ with $\diam \overline{B}_i < \delta$ such that 
    \begin{align*}
    ...
    \end{align*}
    Also, by Proposition \ref{prop_18}, $U\setminus Z$ can be covered by a family of balls $\{B(r_i)\}$ with radius $r_i$ such that $\sum^\infty_{i=1} r_i^n < \varepsilon$, which implies $\H^n_{\infty}(U\setminus Z) = 0$. Hence, $\H^n_{\delta}(U\setminus Z) = 0$. Thus,
    \begin{align*}

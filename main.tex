...
Let $\omega_s = \pi^{s/2} / \Gamma(1 + s/2)$, $s \geq 0$, $\Gamma$ is Euler gamma function. Note that $\omega_0 = 1$ and if $s = n \in \mathbb{N}$, then $\omega_n$ is volume of the unit ball\footnote{See Appendix \ref{appendix_a}.} in $\mathbb{R}^n$. And the volume of ball $B = B^n(x,r) \subset \mathbb{R}^n$ equals 
\begin{align*}
...
\end{align*}
where $\Gamma$ is Euler gamma function in Section \ref{hausdorff_measure}. We want to prove that if $s = n \in \mathbb{N}$, then $\omega_n$ is volume of the unit ball in $\mathbb{R}^n$. First, the Euler gamma function is defined as
\begin{align*}
...
~\begin{enumerate}[label=(\alph*)]
    ...
    \begin{lemma} The integral of the Gaussian function $f(x) = e^{-x^2}$ is
    $\displaystyle \int^\infty_{-\infty} e^{-x^2}\, dx = \sqrt{\pi}$ or $\displaystyle \int^\infty_0 e^{-x^2}\, dx = \frac{\sqrt{\pi}}{2}$.
    \end{lemma}
    \begin{proof}
    A standard way to compute the Gaussian integral is to make use of the property that\footnote{Proof source: \url{https://www.wikiwand.com/en/Gaussian_integral}.}:
    \begin{align*}
    ...
    Now for the gamma function, let $t = s^2$, then $\displaystyle \Gamma(x) = 2 \int^\infty_0 s^{2x-1} e^{-s^2}\, ds$, and by lemma, hence $\displaystyle \Gamma(1/2) = \int^\infty_{-\infty} e^{-s^2}\, ds = \sqrt{\pi}$.
\end{enumerate}
\end{proof}
...
\begin{proof}
~\begin{enumerate}[label=(\alph*)]
    \item Considering the unit ball in $3$ dimensional would be helpful, and the horizontal view is shown below:
    \begin{figure}[H]
        \centering
    ...
    \end{align*}
    If $a_n = 2\pi^{n/2}/(n \Gamma(n/2))$, then $a_1 = 2 = \omega_1$, and $a_n$ satisfies the same recurrence as $\omega_n$. Thus, $\omega_n = a_n$ for all $n = 1,2,\cdots$.
